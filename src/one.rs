...
pub fn example_with_vec() {
    ...
    let string_vec: Vec<String> = (0..NUM_TAGS)
        .enumerate()
        .map(|(i, _)| {
            match i % 3 {
                0 => "us-east",
    ...
    let size: usize = string_vec
        .iter()
        .map(|s| s.len() + mem::size_of::<String>())
        .sum();
    println!(
        "created array with {} elements in {}, size {}",
        string_vec.len(),
        t.done(),
        size,
    );
    ...
    for _ in 0..10 {
        let t = RAAITimer::default();
        let not_west_bitset: Vec<bool> = string_vec.iter().map(|s| s != "us-west").collect();

        println!(
            "Completed finding bitset: {} elements in {}",
            not_west_bitset.len(),
            t.done()
        );
    }
...
pub fn example_with_arrow() {
    ...
    (0..NUM_TAGS).enumerate().for_each(|(i, _)| {
